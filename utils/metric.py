...
class SCELoss(CELoss):
    ...
    def loss(self, output, labels, n_bins = 15, logits = True):
        ...
        print("INSIDE SCELOSS")
        print(f"n_class: {self.n_class} | {type(self.n_class)}")
        print(f"labels: {labels} | {type(labels)}")
        print(f"Size of bins | n_data | n_bins: {bin_n} | {self.n_data} | {self.n_bins}")
